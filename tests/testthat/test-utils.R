test_that("utils", {
  expect_identical(
    length(sequence(as.POSIXct("2001-01-02 03:04:06", tz = "UTC"))),
    1L
  )
  expect_identical(
    length(sequence(as.POSIXct("2001-01-02 03:04:06", tz = "UTC") + c(1, 3))),
    2L
  )
  expect_identical(
    length(sequence(
      as.POSIXct("2001-01-02 03:04:06", tz = "UTC") + c(1, 3, 9)
    )),
    5L
  )
  expect_identical(
    length(sequence(
      as.POSIXct("2001-01-02 03:04:06", tz = "UTC") + c(1, 2, 9)
    )),
    9L
  )
})
...
test_that("size_gaps", {
  ...
  expect_identical(
    size_gaps(c(TRUE, TRUE, FALSE, FALSE, TRUE)),
    c(2L, 2L, 0L, 0L, 1L)
  )
})
